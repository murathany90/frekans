...
try {
  ...
  if (dialogs.length) {
    throw new Error(`Continental Europe-only daily date triggered blocking dialog: ${dialogs.join(" | ")}`);
  }
  ...
  if (result.selectedDate !== targetDate || result.renderedDate !== targetDate) {
    throw new Error(`Continental Europe-only date did not render: ${JSON.stringify(result)}`);
  }
  if (!result.hasDe || result.deFinite < 80000 || result.countB < 80000) {
    throw new Error(`Continental Europe series is not available on the daily page: ${JSON.stringify(result)}`);
  }
  if (result.hasTr || result.trFinite !== 0 || result.countA !== 0 || result.pairedCount !== 0) {
    throw new Error(`Continental Europe-only daily page should keep missing Türkiye data empty: ${JSON.stringify(result)}`);
  }
  if (!result.seriesNames.some(name => /Kıta Avrupası|Continental Europe/i.test(name))) {
    throw new Error(`Continental Europe chart series is missing: ${JSON.stringify(result)}`);
  }
